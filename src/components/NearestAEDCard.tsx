...
export function NearestAEDCard({ userLocation }: NearestAEDCardProps) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    filter((aed) => aed.status !== 'outOfService').
    map((aed) => ({
      ...aed,
  ...
  const statusColor =
  nearestAED.status === 'available24_7' ? 'bg-green-500' : 'bg-yellow-500';
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
      <div className="bg-red-600 text-white px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`${statusColor} w-3 h-3 rounded-full animate-pulse`}>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <a
            ...
            className="flex items-center justify-center bg-blue-600 text-white py-4 rounded-xl font-bold text-base hover:bg-blue-700 transition-colors active:scale-95 transform">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center bg-cyan-500 text-white py-4 rounded-xl font-bold text-base hover:bg-cyan-600 transition-colors active:scale-95 transform">
